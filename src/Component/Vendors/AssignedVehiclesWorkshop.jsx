...
const AssignedVehicleWorkshop = () => {
  ...
  useEffect(() => {
    ...
    if (GetDataOfUser && GetDataOfUser.vendorCode) {
      const fetchAssignedCases = async () => {
        ...
        try {
          const response = await axios.get(`${backendUrl}/api/assignedTasksWorkshop/${GetDataOfUser.vendorCode}`);
  ...
  const findUserById = async (id) => {
    try {
      ...
      // Optionally handle the error by showing a message to the user, etc.
    }
  }
                ...
                <tr key={item.id} >
                  ...
                  <td>
                    {item.details.length > 0 ? (
                      ...
                      onClick={() => view(item.AccidentVehicleCode, item.accidentFileNo)}
