...
import AddedDataByCrain from './AddedDataByCrain';
...
const AssignedVehicleCrain = () => {
  ...
  const fetchAssignedCases = async (vendorCode) => {
    ...
    try {
      const response = await axios.get(`${backendUrl}/api/assignedTasksCrain/${vendorCode}`);
  ...
  return (
    ...
    <div>
      ...
      {showAddedDataByVendor && (
        <AddedDataByCrain id={selectedId} item={selectedItem} onUpdate={handleUpdate}/>
      )}
    </div>
  );
