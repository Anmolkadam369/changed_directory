    ...
    const getData = async (e) => {
        ...
        if (response.data.message === "No accident vehicle data found.") {
            ...
            let filteredData = response.data.data.filter((info) => 
                info?.[`${currentService}Details`]?.confirmDoneWorking === 1 &&
                info?.[`${currentService}Details`]?.customerAcceptedVendor === 1 &&
                info?.[`${currentService}Details`]?.cancelOrder === 0 &&
                info?.[`${currentService}Details`]?.cancelPreAssignOrder === 0 &&
                info?.[`${currentService}Details`]?.completeOrder === 1
            );
            
            let notCompleted = response.data.data.filter((info) =>
                info?.[`${currentService}Details`]?.confirmDoneWorking === 0 &&
                info?.[`${currentService}Details`]?.cancelOrder === 0 && 
                info?.[`${currentService}Details`]?.cancelPreAssignOrder === 0 &&
                info?.[`${currentService}Details`]?.completeOrder === 0
            );
                                            ...
                                            <div style={{
                                                ...
                                                <div style={{ background: "20px" }}>
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
                                                        ...
                                                        <div style={{ marginTop: "5px", marginRight: "10px", width: "35px", background: '#ccb300', border: "1px solid red", borderRadius: "5px", fontSize: "12px", display: "flex", alignItems: "center", justifyContent: 'center', color: 'black' }}>{avg[dataIndex] ? avg[dataIndex] : 0}</div>
                                                    </div>
                                                    <div style={{ display: "flex", alignItems: "center", margin: '7px 5px 0px 5px' }}>
                                                        ...
                                                        <span style={{ color: "green", marginLeft: "5px", fontSize: "12px" }}>{item?.[`${currentService}Details`]?.systemDate?.split("T")[0]}</span>
                                                    </div>
