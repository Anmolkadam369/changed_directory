...
const QuotationUpdate = ({ vehicleNumber }) => {
    ...
    useEffect(() => {
        ...
        if (messages.length > 0) {
            // Filter out all messages that are NOT for 'quotation-updates-crane'
            const remainingMessages = messages.filter((message) => message.forPage !== 'quotation-updates-crane');

            // Flash the messages before removing
            messages.forEach((message) => {
                console.log("messages123", message);
                if (message.forPage === 'quotation-updates-crane') {
                    if (message.message === "vendor-not-found") {
                        setAlertMessage("Currently vendor not found. Please try again later...");
                        setAlertType("danger");
                    } else {
                        getData();
                    }
            ...
            // ✅ Remove all messages for 'quotation-updates-crane'
            messages.length = 0;
    ...
    const getData = async (e) => {
        ...
        if (response.data.message === "No accident vehicle data found.") {
            ...
            console.log("responsjdlfjskldfgjlse123421", response.data.data);
            ...
            let filteredImportant = filteredData.filter((info) =>
                info?.[`${currentService}Details`]?.connectedVendorFully === 1
            )

            let filteredLessImportant = filteredData.filter((info) =>
                info?.[`${currentService}Details`]?.connectedVendorFully === 0
            )
                            ...
                            >
                                {data.map((item, dataIndex) => (
                                    <div style={{
                                        ...
                                        {item?.[`${currentService}Details`] && (
                                            <div style={{
                                                marginTop: '20px', borderTop: '1px solid grey', background: '#dededee6',
                                                borderRadius: "20px",
                                                padding: '1px'
                                            }}>
                                                ...
                                                <div>
                                                    <div className="flex justify-between m-3" >
                                                        ...
                                                        {Boolean (item?.[`${currentService}Details`]?.connectedVendorFully && userId.startsWith('CC-')) && (
                                                            <div
                                                                className="bg-green-700 m-1 px-6 py-2 rounded-xl cursor-pointer"
                                        ...
                                        <div className='px-1 py-1 '>
                                            ...
                                            <div className='flex justify-between pr-3 pt-2'>
                                                <div className='flex items-center space-x-4 top-0'>
                                                    <GarageIcon className='text-white h-4 m-2 mt-0' />
                                                    <p className='text-white text-sm font-semibold'>{currentService} is on the way </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className='flex justify-between  pt-2'>
                                                <div className='flex items-center space-x-4 top-0'>
                                                    ...
                                                    <p className='text-white text-sm font-semibold'>Estimated Total Distance</p>
                                                </div>
                                                <div className='bg-white text-xs font-semibold px-3 py-1 mr-3  rounded-xl'>
                                                    {currentItem?.[`${currentService}Details`].distance} Km
                                                </div>
                                            ...
                                            <div className='flex justify-between  pt-2'>
                                                <div className='flex items-center space-x-4 top-0'>
                                                    <AssistantDirectionIcon className='text-white h-4 m-2 mt-0' />
                                                    <p className='text-white text-sm font-semibold'>Estimated Time To Drop</p>
                                                </div>
                                                <div className='bg-white text-xs font-semibold px-3 py-1 mr-3 mb-3 mt-1 rounded-xl'>
                                                    {currentItem?.[`${currentService}Details`].duration}
                                                </div>
