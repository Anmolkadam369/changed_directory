...
export default function AllVehicles() {
    ...
    const getFilteredData = (filter) => {
        ...
        for (let i = 0; i < accidentData.length; i++) {
            console.log('accidentData[i]?.systemDate.split', accidentData[i]?.[activeCategory !== 'all' ? `${activeCategory}Details` : 'craneDetails']?.systemDate?.split('T'))
            console.log('accidentData[i]?.systemDate.split', accidentData[0]?.[activeCategory !== 'all' ? `${activeCategory}Details` : 'craneDetails']?.systemDate?.split('T'))
            let getTime = accidentData[i]?.[`${activeCategory}Details`]?.systemDate.split('T');
            let assignedDate = getTime[0];
            let assignedTime = getTime[1];
            console.log('assignedate', getTime[0], getTime[1])
        ...
        if (filter === 'newest') {
            ...
            accidentData.sort((a, b) => {
                const dateA = new Date(a?.systemDate.split('T').join(' '));
                const dateB = new Date(b?.systemDate.split('T').join(' '));
    ...
    useEffect(()=>{
        ...
        console.log('data1', accidentVehicleData)
        if(activeCategory == 'all' ) {
            setData(accidentVehicleData);
            return;
        }
        const getActiveCategory = accidentVehicleData.filter((item)=>{
            console.log('item1', item)
           const selectedOptions1 = item.selectedOptions.split(',')
           console.log('selectedoption', selectedOptions1)
           return selectedOptions1.some((option) => {
                console.log('item[`${option}Status`] ! && option === activeCategory', item[`${option}Status`] , 'cancelled' , option ,activeCategory)
              return item[`${option}Status`] !== 'cancelled' && option === activeCategory
            })

        //     const arrayItem = item.selectedOptions.split(',')
        //    if( item.selectedOptions.includes(activeCategory)){
        //     console.log('item2', item)
        //      return item;
        //     }
        })
        setData(getActiveCategory)
    },[activeCategory])
                        ...
                        <div style={{ marginBottom: "30px" }}>
                            {selectedIndex == 0 && (
                                <div className="px-4 mb-6 mt-4">
                                    <div className="flex flex-wrap space-x-2 gap-2 pb-2 scrollbar-hide max-full ">
                                        {[
                                            { icon: Truck, label: "All Cases", category: "all" },
                                            { icon: Truck, label: "Recovery", category: "recoveryVan" },
                                            { icon: PrecisionManufacturingIcon, label: "Crane", category: "crane" },
                                            { icon: Scale, label: "Legal", category: "advocate" },
                                            { icon: Wrench, label: "Repairs", category: "repairs" },
                                            { icon: Building2, label: "Workshops", category: "workshops" }
                                        ].map((item) => (
                                            <CategoryPill
                                                key={item.category}
                                                icon={item.icon}
                                                ...
                                                className="text-xs px-2 py-1 w-auto mt-1"
                                            />
                                        ))}
                                    </div>
                                    ...
                                    {data.map((item, index) => (
                                        <div key={index}>
                                            <div style={{
                                                ...
                                                border: '1px solid red', borderTop: 'none', borderRight: 'none'
                                            }} className=' w-[90%]  border-[#4b6c6d] shadow-[rgba(75,_108,_109,_0.2)]  rounded-xl m-2'>
