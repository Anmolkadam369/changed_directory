...
const VendorSignUp = () => {
  ...
  const generateFile = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/dBToExcel/${userId}/vendor`,{ headers: { Authorization: `Bearer ${token}` }});
  ...
  const columns = [
    ...
    {
      name: "Pre Approval", selector: (row) => row.first_approval, sortable: true, width: "150px",
      cell: (row) => (
        <span style={{ color: 'brown' }}>{row.first_approval ?row.first_approval.charAt(0).toUpperCase() + row.first_approval.slice(1) : ''}</span>
      )
    },
    {
      name: "Fianl Approval", selector: (row) => row.second_approval, sortable: true, width: "150px",
      cell: (row) => (
        <span style={{ color: 'brown' }}>{row.second_approval ? row.second_approval.charAt(0).toUpperCase() + row.second_approval.slice(1):""}</span>
      )
    },
    {
  ...
  return (
    <div>
      ...
      {showVendorTable && (
        <div className="Customer-master-form" style={{ marginLeft: '10px', paddingLeft: '0px', marginRight: '10px', paddingRight: '0px' }}>
          <Helmet>
            ...
            <meta name="keywords" content="Vehicle Accidents, accident trucks, vendor service, Customer Service, Claimpro, Claim pro Assist, Bvc Claimpro Assist, Accidental repair, Motor Insurance claim, Advocate services, Hydra Crane service, On site repair, Accident Management" />
            <link rel='canonical' href={`https://claimpro.in/VendorApporoved`} />
          </Helmet>

          <div>
            <h3 className="bigtitle">Sign-up Vendor</h3>
      ...
      {showVendorMasterEdit && (
        <VendorMasterEdit id={selectedId} onUpdate={handleUpdate} pageFrom = 'VendorSignUp'/>
      )}
    </div>
  );
};
